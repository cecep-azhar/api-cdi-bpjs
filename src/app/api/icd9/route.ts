...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await db.select().from(icd9).orderBy(icd9.code);
  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      for (const val of values) {
        await db.insert(icd9).values(val).onConflictDoUpdate({
          target: icd9.code,
      ...
      const result = await db.insert(icd9).values({ code, nameId, nameEn, isActive }).onConflictDoUpdate({
        target: icd9.code,
